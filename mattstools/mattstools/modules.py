...
class SingleLinear(nn.Linear):
    def __init__(
        self, inpt_dim: int, outp_dim: int, edge_dim=None, ctxt_dim=None, **kwargs
    ) -> None:
        super().__init__(inpt_dim, outp_dim, **kwargs)

    def forward(self, nodes, mask, ctxt=None, attn_bias=None, attn_mask=None):
        output = super(SingleLinear, self).forward(nodes) * mask.unsqueeze(-1)
        # return prediction.sum(1) / mask.sum(-1, keepdims=True)
        return (output * mask.unsqueeze(-1)).sum(-2) / mask.sum(1, keepdim=True)
...
class NewIterativeNormLayer(nn.Module):
